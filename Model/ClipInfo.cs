...
namespace moe.yo3explorer.sharpBluRay.Model
{
    ...
    {
        public ClipInfo(byte[] buffer, int id)
        {
            ...
            ParseClipInfoEntity(entityBuffer);
        ...
        private void ParseClipInfoEntity(byte[] buffer)
        {
            MemoryStream ms = new MemoryStream(buffer, false);
            ms.Position += 2;
        ...
        {
            return $"{nameof(ID)}: {ID}, {nameof(FormatId)}: {FormatId}, {nameof(AtcDeltaCount)}: {AtcDeltaCount}, {nameof(TsRecordingRate)}: {TsRecordingRate}, {nameof(NumSourcePackets)}: {NumSourcePackets}";
        }
    }
}
